...
void TWindow::initializeGL()
{
    ...
    m_cube = std::make_shared<Cube>(this);
     ...
     m_cube2 = std::make_shared<Cube>(this);
      m_cube3 = std::make_shared<Cube>(this);
    m_skyBox = std::make_shared<SkyBox>(this);
    myearth= std::make_shared<earth>(this);
    startTimer(1000 / 60);
...
void TWindow::paintGL()
{
    ...
    matrix.rotate(2*rotation_y,0.0,1.0,0.0);
    matrix.translate(0.0, 0.0, -0.7);
    matrix.scale(0.05,0.05,0.05);
    m_model = matrix;
    m_cube->draw(m_program, m_model, m_view, m_project);
    QMatrix4x4 matrix1;
    matrix1.rotate(2*rotation_y,0.0,0.0,1.0);
    matrix1.translate(0.0, 0.7, 0.0);
    matrix1.scale(0.05,0.05,0.05);
    m_cube1->draw(m_program, matrix1, m_view, m_project);
    QMatrix4x4 matrix2;
    matrix2.rotate(0.5*rotation_y,0.0,1.0,0.0);
    matrix2.translate(0.0, 0.0, 0.0);
     matrix2.scale(0.05,0.05,0.05);

   myearth->draw(m_program, matrix2, m_view, m_project);
   m_skyBox->draw(m_program, {}, QMatrix4x4(m_view.normalMatrix()), m_project);
      QMatrix4x4 matrix3;
      matrix3.rotate(2*rotation_y,0.0,1.0,1.0);
      matrix3.translate(0.0, -0.7, 0.0);
      matrix3.scale(0.05,0.05,0.05);
      m_cube2->draw(m_program, matrix3, m_view, m_project);
      QMatrix4x4 matrix4;
      matrix4.rotate(2*rotation_y,1.0,1.0,0.0);
      matrix4.translate(0.7, 0.0, 0.0);
      matrix4.scale(0.05,0.05,0.05);
      m_cube3->draw(m_program, matrix4, m_view, m_project);
